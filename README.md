...
想不想在网络上拥有完全属于自己的 “ 私人领地 ” ：电能进，字节能进，国王不能进！

一个私人博客网站，足够简单、实用、美观。去中心化抗审查、100%掌握自己数据，除了你自己没人能控制它。

这个项目是开源的。只要下载代码，然后按教程使用命令行部署到 IC 上就行了。代码结构非常简单，只有一个前端和一个后端。

定位在独立的 web3 个人网站，类似 Web2 的 WordPerss + 阿里云。区别是用户自己可以控制自己的所有数据，自由创建网站，抗审查，的 IC 云端博客网站。

社区可以自己修改前端页面，做不同风格的主页，欢迎 pr 。
